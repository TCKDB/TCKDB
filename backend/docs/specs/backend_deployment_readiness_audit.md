...
| P0-2 | Migration policy drift — `.claude/rules/migration-rules.md` says "single migration only" but 4 migrations exist (`60b67e360daf`, `d861dfd60891`, `a1b2c3d4e5f6`, `b2c3d4e5f6a7`) | [backend/alembic/versions/](backend/alembic/versions/), `.claude/rules/migration-rules.md` | Update the rule to reflect Phase 4a relaxation; document the deployed-DB migration flow (init vs upgrade). |
...
| P1-2 | No FK indexes on hot Calculation filter columns: `lot_id`, `software_release_id`, `conformer_observation_id`, `species_entry_id`, `transition_state_entry_id`, `literature_id` | [backend/app/db/models/calculation.py:71-107](backend/app/db/models/calculation.py#L71-L107) | **Addressed (2026-05-23).** Indexes added via additive migration `c3d4e5f6a7b8_add_calculation_fk_indexes` and `index=True` on the model columns. Follow-up (2026-05-24): the pre-launch performance review flagged that `calculation.workflow_tool_release_id` and the per-product-table `literature_id` columns (`thermo`, `kinetics`, `statmech`, `transport`, `network`, `network_solve`) were still unindexed — addressed via migration `e5f6a7b8c9d0_add_literature_and_workflow_fk_indexes`. Follow-up (2026-05-24): the same review flagged `calculation_artifact.calculation_id` / `kind` / `sha256` as the next unindexed hot columns on the `/scientific/artifacts/search` surface — addressed via migration `f6a7b8c9d0e1_add_calculation_artifact_indexes`. |
...
| P1-9 ✅ addressed | Chebyshev / PLOG coefficient payload has no explicit truncation cap | [backend/app/services/scientific_read/network_kinetics.py](backend/app/services/scientific_read/network_kinetics.py) | **Done.** Chebyshev `include=coefficients` flattens in `(temperature_order, pressure_order)` order and is capped at `settings.public_max_limit`; payload exposes `coefficient_count_total` + `coefficients_truncated`. PLOG `include=plog` stays a bare `record.plog` list capped at the same limit, with sibling `record.plog_entry_count_total` + `record.plog_entries_truncated` carrying the truncation metadata. `include=all` still excludes `points`. |
...
| Artifact search metadata filters | Btree indexes on `calculation_artifact.calculation_id` / `kind` / `sha256` added in `f6a7b8c9d0e1_add_calculation_artifact_indexes`; `filename_contains` remains a `LIKE` scan pending a future generated/trigram design | Safe for v0 |
| Review badge bulk loading | Bulk-loaded in batches | Safe for v0 |
| Literature inverse records | Bounded by literature.id filter; per-product-table `literature_id` btree indexes added in `e5f6a7b8c9d0_add_literature_and_workflow_fk_indexes` | Safe for v0 |
...
**Gap closed**: P1-9 above — Chebyshev / PLOG payloads are now capped at `settings.public_max_limit` with truncation metadata. Chebyshev surfaces `coefficient_count_total` / `coefficients_truncated` inside the existing `coefficients` payload; PLOG keeps the bare `record.plog` list and exposes sibling `record.plog_entry_count_total` / `record.plog_entries_truncated` on the record itself.
