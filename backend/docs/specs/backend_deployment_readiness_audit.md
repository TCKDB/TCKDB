...
| P1-2 | No FK indexes on hot Calculation filter columns: `lot_id`, `software_release_id`, `conformer_observation_id`, `species_entry_id`, `transition_state_entry_id`, `literature_id` | [backend/app/db/models/calculation.py:71-107](backend/app/db/models/calculation.py#L71-L107) | Add `index=True` on each FK column. Fold into `d861dfd60891` per existing rule (or into a new additive migration if Phase 4a relaxation is formalized). |
