...
4. **Canonical pressure query — implemented.** `pressure_bar` is canonical on reaction-entry and chemistry-first kinetics reads; `pressure` remains a deprecated alias. Equal values are canonicalized to `pressure_bar`. Unequal values return 422 `pressure_alias_conflict` for both GET and POST, with both submitted values retained in the validation detail (the context object is currently empty). A finite pressure includes pressure-independent rates; matches exact apparent-pressure records; applies bounded PLOG/Chebyshev coverage; accepts populated falloff/third-body models; and excludes high-pressure-limit, out-of-range, or indeterminate pressure-dependent records. Incompatible records are filtered out rather than silently broadened. Every non-null `reaction_path_degeneracy` states `reported_rate_coefficient_includes_degeneracy=true` and `apply_to_rate_coefficient=false`: the returned rate already includes the stored degeneracy and consumers must not apply it again. Null degeneracy remains unknown, not one.
