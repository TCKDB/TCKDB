...
2. **Fail closed on ignored filters — implemented.** Non-null `inchi` on species search (including composed thermo and species-calculation search), species-calculation `scientific_origin`, frequency-scale-factor `model_kind`/`software_version`, and energy-correction-scheme `used_by_thermo` return 422 `unsupported_filter`. (Energy-correction-scheme `software`/`software_version` were lifted once the row gained a `software_release_id` join — plan v1/v2 — and are now implemented rather than rejected.) No subset of a request is silently applied.
