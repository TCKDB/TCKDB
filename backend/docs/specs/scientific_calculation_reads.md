...
| `spin_diagnostic` | implemented | Singleton list (`calculation_id` PK on the table); ORM-backed scalar spin-contamination diagnostics (`s_squared`, `s_squared_expected`, `s_squared_annihilated`). Absence reads as "not parsed / not applicable / not reported" (typically restricted / closed-shell runs). Heuristic thresholds for ⟨S²⟩ deviation are deliberately not enforced or labelled by the database — readers and curators apply interpretation. |
...
| `imaginary_mode_projections` | implemented | **A read-time determination, not a stored record.** For each imaginary mode in `calc_freq_mode`, projects the eigenvector recovered from `calc_hessian` onto the rigid-body subspace (translations + rotations about the centre of mass) and onto a dihedral rotation about each perceived rotatable bond, per ADR 0012. Nothing is persisted: the block is recomputed per request from the stored matrix and the geometry it is bound to. Carries the depositor's declared `imaginary_disposition` **beside** the computed `determination`, with the raw overlaps and the thresholds applied, plus an `agreement` field (`not_declared` / `agrees` / `conflicts` / `inconclusive`) — a disagreement is surfaced, never resolved. The determination vocabulary is deliberately narrower than the declared enum (`rigid_body_residue`, `torsion`, `internal_vibration`). Unlike every other optional block this one is **never empty and never null once requested**: `status` distinguishes `determined` from `hessian_not_stored` / `no_imaginary_modes` / `no_frequency_modes` / `geometry_incomplete` / `masses_unresolved` / `rigid_body_curvature_too_large`, because "nothing was checked" and "nothing was wrong" are opposite findings. A not-determinable status is not a shrug either: it carries `tau_context`, ADR 0012's judgement as **stored** on `calc_freq_result` — the tau applied, the `tau_basis` row of the protocol table it came from, the `calculation_parameter` values that chose that row (all three `TAU_PARAMETER_KEYS`, absences included, because an absent key is usually what decided the threshold), the persisted `structural_flag`, and every imaginary mode ranked by descending magnitude with a bare `at_or_above_tau` comparison. Nothing in it is re-resolved — ADR 0012 requires tau to be stored so a parser improvement cannot silently re-label a historical record — and nothing in it labels a mode: magnitude cannot separate a spurious mode from a real one that is not the reaction coordinate, so the block reports the comparison, states that limit in `interpretation_limit`, and stops. `tau_context` is `null` on a `determined` block, because ADR 0012 says a determination beats a threshold wherever one is available. Cost is one small dense eigendecomposition (3N × 3N, N ≤ 30 in practice), which is why it is **detail-only and opt-in by name**: like `trust`, it stays out of search and out of `include=all`'s expansion. Measured at 0.45 ms for a 12-atom molecule and growing as N^3. |
...
| `imaginary_mode_projections` | `imaginary_mode_projections: ImaginaryModeProjectionSummary` (always present once requested; `status` says whether a projection was taken and, if not, what stopped it; `tau_context` carries ADR 0012's stored magnitude judgement when it was not) | `calc_hessian` + `geometry_atom` + `calc_freq_mode`, computed at read time; `tau_context` read back from `calc_freq_result` + `calculation_parameter` |
...
| `GET /api/v1/scientific/species-calculations/search` | Stays as the species-rooted search. New endpoint is owner-agnostic and adds method/basis/validation/parameter filtering. The two are complementary, not redundant — species-calculations preserves the per-record `species` context block and `lowest_energy` ranking; the new endpoint focuses on calculation-as-evidence. **It now serves frequency results too**, reusing this spec's `CalculationFreqResultSummary` and `CalculationFreqModeSummary` fragments and the same `include=freq_modes` token, so one table has one public shape across the two surfaces. It projects them as a top-level `frequency` block beside `energy` rather than under a `results` envelope, because that surface has never had a `results` envelope. Contract: [`docs/specs/species_calculation_search_api.md`](../../../docs/specs/species_calculation_search_api.md). | Keep both. |
