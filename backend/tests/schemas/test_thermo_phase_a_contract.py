...
def test_nonfinite_scalar_refused(schema, value, field):
    extra = {"species_entry": IDENTITY} if schema is ThermoUploadRequest else {}
    with pytest.raises(ValidationError):
        schema(**extra, **{"enthalpy_reference_kind": "formation_from_elements_298k", "h298_kj_mol": 0, field: value})
...
def test_nasa9_invalid_coverage(intervals):
    with pytest.raises(ValidationError):
        ThermoUploadRequest(enthalpy_reference_kind="formation_from_elements_298k", species_entry=IDENTITY, nasa9_intervals=intervals)


def test_nasa9_index_order_and_auxiliary_points():
    record = ThermoUploadRequest(enthalpy_reference_kind="formation_from_elements_298k", species_entry=IDENTITY,
