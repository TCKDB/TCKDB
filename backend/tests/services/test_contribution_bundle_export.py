...
def _seed_thermo(session: Session, *, smiles: str, note: str) -> int:
    ...
    request = ThermoUploadRequest(
        enthalpy_reference_kind="formation_298k", species_entry={"smiles": smiles, "charge": 0, "multiplicity": 1},
...
def test_export_thermo_bundle_carries_provenance_when_present(db_engine) -> None:
    ...
    with _isolated_session(db_engine) as session:
        request = ThermoUploadRequest(
            enthalpy_reference_kind="formation_298k", species_entry={"smiles": "CO", "charge": 0, "multiplicity": 1},
            scientific_origin="computed",
            h298_kj_mol=-200.7,
            s298_j_mol_k=239.7,
