...
def test_rejects_a_limit_above_the_service_cap(client, db_session, monkeypatch):
    ...
    This test sent ``?limit=999`` and asserted ``422``. ``Query(le=200)``
    on the route refuses that at the framework boundary, so the service
    was never called: the test was named after a pagination code it
    could not produce, and asserted nothing about it. Both bounds are
    exercised here -- the outer one still returns
    ``request_validation_error``, and a limit *inside* it but above the
    hosted cap reaches the service and returns ``limit_too_large``.
    """
    _, entry = _entry(db_session)
