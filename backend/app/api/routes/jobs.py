...
from app.db.models.common import AppUserRole, UploadJobKind, UploadJobStatus
...
) -> JobEnqueueResponse | JSONResponse:
    """Create or replay a queue submission under the shared HTTP contract."""
    if (replay := idem.maybe_replay()) is not None:
        return replay
    result = _enqueue(session, kind, request, user_id)
    idem.record(session, status_code=202, body=result.model_dump(mode="json"))
    return result
...
def enqueue_computed_reaction(
    ...
    return _enqueue_idempotent(session, UploadJobKind.computed_reaction, request, current_user.id, idem)
...
def enqueue_conformer(
    ...
    return _enqueue_idempotent(session, UploadJobKind.conformer, request, current_user.id, idem)
...
def enqueue_reaction(
    ...
    return _enqueue_idempotent(session, UploadJobKind.reaction, request, current_user.id, idem)
...
def enqueue_kinetics(
    ...
    return _enqueue_idempotent(session, UploadJobKind.kinetics, request, current_user.id, idem)
...
def enqueue_network(
    ...
    return _enqueue_idempotent(session, UploadJobKind.network, request, current_user.id, idem)
...
def enqueue_network_pdep(
    ...
    return _enqueue_idempotent(session, UploadJobKind.network_pdep, request, current_user.id, idem)
...
def enqueue_thermo(
    ...
    return _enqueue_idempotent(session, UploadJobKind.thermo, request, current_user.id, idem)
...
def enqueue_transition_state(
    ...
    return _enqueue_idempotent(session, UploadJobKind.transition_state, request, current_user.id, idem)
...
def enqueue_transport(
    ...
    return _enqueue_idempotent(session, UploadJobKind.transport, request, current_user.id, idem)
...
def get_job_status(
    ...
    if job is None or (job.created_by != current_user.id and current_user.role not in _CURATION_ROLES):
