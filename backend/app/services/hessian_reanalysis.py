...
read, derived from the printed precision of the two artefacts being
compared rather than from a measured corpus figure. (A figure of
0.045 cm^-1 was recorded for the 2026-08-11 corpus by the unprojected
read-time matcher; it is a measurement, and it is not used as the bound.)

Two roundings separate the stored list from the stored matrix:

1. **The frequency list is printed to a fixed number of decimals.**
   Gaussian prints four (``Frequencies -- 110.1603``), ORCA and Molpro
   print two. The coarsest parser TCKDB accepts therefore contributes a
   half-ULP of :data:`FREQUENCY_PRINT_HALF_ULP_CM1` = 0.005 cm^-1, flat
   in omega.

2. **The force constants are printed to a fixed number of significant
   figures.** Gaussian's log block is six (``0.410282D-01``), the
   coarsest of the three formats parsed (Molpro prints seven decimals,
   ORCA ten). An element rounding of relative size
   :data:`HESSIAN_PRINT_RELATIVE_HALF_ULP` = 5e-7 on force constants of
   order :data:`HESSIAN_ELEMENT_SCALE_HARTREE_BOHR2_AMU` = 1 (an X-H
   stretch diagonal is ~0.7 hartree/bohr^2 over ~1 amu) perturbs a
   mass-weighted eigenvalue by up to 5e-7 hartree/bohr^2/amu, which the
   module's own constants convert to
   :data:`DEFAULT_MAX_OMEGA2_DEVIATION_CM2` ~ 13.2 cm^-2. This term is
   flat in **omega squared**, not in omega -- ADR 0012's observation --
   so it is 0.002 cm^-1 on a 3000 cm^-1 stretch and 0.06 cm^-1 on a
   110 cm^-1 torsion. Measured on ``freq_g09.log``, projected: the
   largest omega-squared deviation over 30 modes is 11.2 cm^-2, inside
   the bound with no fitting.

A stored mode therefore passes when

    |w_rec - w_stored| <= max_deviation_cm1 + (sqrt(w_stored^2 + B) - |w_stored|)

with ``max_deviation_cm1`` = :data:`DEFAULT_MAX_DEVIATION_CM1` (item 1)
and ``B`` = :data:`DEFAULT_MAX_OMEGA2_DEVIATION_CM2` (item 2); the second
term is the exact inverse of ``|w_rec^2 - w_stored^2| <= B``. Passing
``max_omega2_deviation_cm2=0`` reduces it to a flat cm^-1 bound.
...
2e-5 cm^-1 at 4000 cm^-1 and below either item above. ORCA's ``.hess``
...
atom (1.0 cm^-1 on the C-Cl stretch of ``Orca_TS_test.hess``, 0.35 cm^-1
on the Molpro fixture's X-H stretches, whose printed list appears to use
a different hydrogen mass). Those are reported as exceeding, with the
numbers, because that is what they are: a convention difference the
deposit did not record, not agreement.
...
#: Half-ULP of a six-significant-figure force constant, the coarsest of
#: the three Hessian formats parsed (Gaussian log ``D``-format).
HESSIAN_PRINT_RELATIVE_HALF_ULP = 5e-7

#: Upper scale of a mass-weighted force constant, hartree/bohr^2/amu. An
#: X-H stretch diagonal element is ~0.7 hartree/bohr^2 over ~1 amu; the
#: largest mass-weighted element of ``freq_g09.log`` is 0.35.
HESSIAN_ELEMENT_SCALE_HARTREE_BOHR2_AMU = 1.0
...
#: Item 1 of the derivation in the module docstring: flat in omega.
DEFAULT_MAX_DEVIATION_CM1 = FREQUENCY_PRINT_HALF_ULP_CM1

#: Item 2 of the derivation: flat in omega squared. ~13.2 cm^-2.
DEFAULT_MAX_OMEGA2_DEVIATION_CM2 = (
...
def mode_tolerance_cm1(
    ...
    *,
    ...
    max_omega2_deviation_cm2: float = DEFAULT_MAX_OMEGA2_DEVIATION_CM2,
) -> float:
    """The bound one stored mode is judged against.

    ``max_deviation_cm1`` plus the exact width, in omega, of an
    omega-squared interval of half-width ``max_omega2_deviation_cm2``
    around the stored value. See the module docstring.
    """

    magnitude = abs(stored_frequency_cm1)
    curvature_term = math.sqrt(magnitude * magnitude + max_omega2_deviation_cm2) - magnitude
...
def compare_spectrum(
    ...
    *,
    ...
    :param max_deviation_cm1: Flat part of the bound.
    :param max_omega2_deviation_cm2: Omega-squared part of the bound.
...
def reanalysis_scope(session: Session, *, calculation_ref: str | None = None) -> list[Calculation]:
    ...
    if calculation_ref is not None:
        if calculation_ref.isdigit():
            statement = statement.where(Calculation.id == int(calculation_ref))
        else:
...
def hessian_reanalysis(
    ...
    *,
    ...
    :param calculation_ref: One calculation (public ref or integer id),
        or ``None`` for :func:`reanalysis_scope`.
    :param max_deviation_cm1: Flat part of the per-mode bound.
    :param max_omega2_deviation_cm2: Omega-squared part of the bound.
    ...
    return {
        ...
        "tolerance": {
            "max_deviation_cm1": max_deviation_cm1,
            "max_omega2_deviation_cm2": round(max_omega2_deviation_cm2, _CM2_DECIMALS),
            "rule": (
                "|recovered - stored| <= max_deviation_cm1 + (sqrt(stored^2 + max_omega2_deviation_cm2) - |stored|)"
            ),
            "derivation": {
                "frequency_print_half_ulp_cm1": FREQUENCY_PRINT_HALF_ULP_CM1,
                "hessian_print_relative_half_ulp": HESSIAN_PRINT_RELATIVE_HALF_ULP,
                "hessian_element_scale_hartree_bohr2_amu": HESSIAN_ELEMENT_SCALE_HARTREE_BOHR2_AMU,
