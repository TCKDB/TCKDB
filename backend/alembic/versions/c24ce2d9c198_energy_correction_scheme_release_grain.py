...
def _refuse_software_only_collisions(bind) -> None:
    ...
    raise RuntimeError(
        ...
        "new identity still checks -- kind, name, level_of_theory_id, "
        "version, units, source_literature_id or "
        "workflow_tool_release_id -- and re-run the upgrade. In practice "
        "that means either giving one scheme a distinguishing `version` "
        "or `name`, or deleting the redundant scheme and repointing its "
        "applied_energy_correction rows at the survivor. Do this "
        "deliberately: which of two same-named libraries is the real one "
        "is a curation decision, not a mechanical one.\n\n"
        "Note that the admin provenance route cannot help here. At this "
        "point the database is still at b6d80e36dcec, where "
        "software_release_id does not yet exist and every colliding row "
        "already has software_id set -- so PATCH "
        "/admin/energy-correction-schemes/{ref}/provenance returns 409 "
        "(already set) against the deployed code, and errors against a "
        "column that is not there with the new code."
    )
