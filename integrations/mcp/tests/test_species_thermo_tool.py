"""Tests for ``tckdb_mcp.tools.species_thermo``."""
...
from tckdb_mcp.tools import species_thermo as st_tool

TOOL_NAME = "tckdb_get_species_entry_thermo"
VALID_REF = "spe_01HZ5K9X2A"
...
def test_dispatch_routes_to_species_thermo_tool() -> None:
    captured: list[dict[str, Any]] = []
    client = _make_client(_ok_handler(captured))
    result = dispatch_tool(
        TOOL_NAME, {"species_entry_ref": VALID_REF}, client, _cfg()
    )
    assert captured[0]["path"] == (
        f"/api/v1/scientific/species-entries/{VALID_REF}/thermo"
    )
...
def test_url_resolves_under_api_v1_with_ref_in_path() -> None:
    ...
    assert captured[0]["url"].startswith(
        f"http://127.0.0.1:8010/api/v1/scientific/species-entries/{VALID_REF}/thermo"
    )
...
def test_rejects_missing_species_entry_ref() -> None:
    client = _make_client(_ok_handler([]))
    with pytest.raises(MCPToolError) as excinfo:
        st_tool.run(client, _cfg(), {})
    assert excinfo.value.code == "invalid_input"
    assert "species_entry_ref" in excinfo.value.detail
    client.close()


def test_rejects_empty_string_species_entry_ref() -> None:
    client = _make_client(_ok_handler([]))
    with pytest.raises(MCPToolError):
        st_tool.run(client, _cfg(), {"species_entry_ref": ""})
    client.close()


def test_rejects_non_string_species_entry_ref() -> None:
    client = _make_client(_ok_handler([]))
    with pytest.raises(MCPToolError) as excinfo:
        st_tool.run(client, _cfg(), {"species_entry_ref": 42})
    assert "must be a string" in excinfo.value.detail
    client.close()


def test_accepts_valid_spe_ref() -> None:
    captured: list[dict[str, Any]] = []
    client = _make_client(_ok_handler(captured))
    st_tool.run(client, _cfg(), {"species_entry_ref": VALID_REF})
    assert captured[0]["path"].endswith(f"/{VALID_REF}/thermo")
    client.close()


@pytest.mark.parametrize(
    "ref",
    ["spc_01HZ5K9X2A", "rxe_01HZ5K9X2A", "rxn_01HZ5", "geom_01HZ"],
)
def test_rejects_wrong_prefix_refs(ref: str) -> None:
    client = _make_client(_ok_handler([]))
    with pytest.raises(MCPToolError) as excinfo:
        st_tool.run(client, _cfg(), {"species_entry_ref": ref})
    assert excinfo.value.code == "invalid_input"
...
def test_rejects_bare_prefix_with_no_body() -> None:
    client = _make_client(_ok_handler([]))
    with pytest.raises(MCPToolError) as excinfo:
        st_tool.run(client, _cfg(), {"species_entry_ref": "spe_"})
    assert "no body" in excinfo.value.detail
    client.close()


def test_rejects_ref_longer_than_64_chars() -> None:
    long_ref = "spe_" + ("A" * 61)  # total 65
    client = _make_client(_ok_handler([]))
    with pytest.raises(MCPToolError) as excinfo:
        st_tool.run(client, _cfg(), {"species_entry_ref": long_ref})
...
def test_path_traversal_attempt_rejected() -> None:
    ...
    with pytest.raises(MCPToolError) as excinfo:
        st_tool.run(
            client, _cfg(), {"species_entry_ref": "spe_../../admin"}
        )
    assert excinfo.value.code == "invalid_input"
    assert "path-unsafe" in excinfo.value.detail
    client.close()
...
@pytest.mark.parametrize(
    "field", ["species_entry_id", "species_id", "level_of_theory_id"]
)
def test_rejects_integer_id_fields_with_teaching_error(field: str) -> None:
    client = _make_client(_ok_handler([]))
    with pytest.raises(MCPToolError) as excinfo:
        st_tool.run(
            client, _cfg(), {"species_entry_ref": VALID_REF, field: 7}
        )
    assert excinfo.value.code == "invalid_input"
...
def test_integer_id_rejection_runs_before_unknown_field_check() -> None:
    ...
    with pytest.raises(MCPToolError) as excinfo:
        st_tool.run(
            client,
            _cfg(),
            {"species_entry_id": 1, "frobnicate": True},
        )
    assert "integer-id" in excinfo.value.detail
    client.close()
...
def test_valid_level_of_theory_ref_forwarded() -> None:
    ...
    st_tool.run(
        client,
        _cfg(),
        {"species_entry_ref": VALID_REF, "level_of_theory_ref": "lot_abc"},
    )
...
def test_rejects_level_of_theory_ref_wrong_prefix() -> None:
    ...
    with pytest.raises(MCPToolError) as excinfo:
        st_tool.run(
            client,
            _cfg(),
            {"species_entry_ref": VALID_REF, "level_of_theory_ref": "spe_abc"},
        )
...
def test_explicit_empty_include_overrides_default() -> None:
    ...
    client = _make_client(_ok_handler(captured))
    st_tool.run(
        client, _cfg(), {"species_entry_ref": VALID_REF, "include": []}
    )
    sent = captured[0]["url"]
...
def test_rejects_internal_ids_include_token() -> None:
    ...
    with pytest.raises(MCPToolError) as excinfo:
        st_tool.run(
            client,
            _cfg(),
            {"species_entry_ref": VALID_REF, "include": ["internal_ids"]},
        )
...
def test_rejects_unknown_include_token() -> None:
    ...
    with pytest.raises(MCPToolError) as excinfo:
        st_tool.run(
            client,
            _cfg(),
            {"species_entry_ref": VALID_REF, "include": ["bogus"]},
        )
    assert "bogus" in excinfo.value.detail
    client.close()


def test_rejects_reaction_kinetics_include_tokens() -> None:
    """``transition_states`` is legal for kinetics, but not for thermo."""
    client = _make_client(_ok_handler([]))
    with pytest.raises(MCPToolError) as excinfo:
        st_tool.run(
            client,
            _cfg(),
            {"species_entry_ref": VALID_REF, "include": ["transition_states"]},
        )
...
def test_each_legal_include_token_accepted(token: str) -> None:
    ...
    st_tool.run(
        client,
        _cfg(),
        {"species_entry_ref": VALID_REF, "include": [token]},
    )
...
def test_multiple_legal_include_tokens_forwarded_as_repeated_params() -> None:
    ...
    st_tool.run(
        ...
        _cfg(),
        {
            "species_entry_ref": VALID_REF,
            "include": ["provenance", "calculations", "statmech"],
        },
    )
    sent = [v for k, v in captured[0]["multi_params"] if k == "include"]
    assert sent == ["provenance", "calculations", "statmech"]
    client.close()
...
def test_limit_capped_to_max() -> None:
    ...
    client = _make_client(_ok_handler(captured))
    st_tool.run(
        client, _cfg(), {"species_entry_ref": VALID_REF, "limit": 10_000}
    )
...
def test_offset_validation_negative_rejected() -> None:
    ...
    with pytest.raises(MCPToolError):
        st_tool.run(
            client, _cfg(), {"species_entry_ref": VALID_REF, "offset": -1}
        )
    client.close()
...
def test_offset_zero_accepted() -> None:
    captured: list[dict[str, Any]] = []
    client = _make_client(_ok_handler(captured))
    st_tool.run(client, _cfg(), {"species_entry_ref": VALID_REF, "offset": 0})
...
def test_legal_collapse_values_forwarded(collapse: str) -> None:
    ...
    st_tool.run(
        client,
        _cfg(),
        {"species_entry_ref": VALID_REF, "collapse": collapse},
    )
...
def test_rejects_unknown_collapse_value() -> None:
    ...
    with pytest.raises(MCPToolError):
        st_tool.run(
            client,
            _cfg(),
            {"species_entry_ref": VALID_REF, "collapse": "every"},
        )
    client.close()
...
def test_rejects_unknown_field() -> None:
    ...
    with pytest.raises(MCPToolError) as excinfo:
        st_tool.run(
            client,
            _cfg(),
            {"species_entry_ref": VALID_REF, "frobnicate": True},
        )
    assert "unknown field" in excinfo.value.detail
    client.close()


def test_no_pressure_field() -> None:
    """Thermo is not pressure-dependent; pressure must not be accepted."""
    client = _make_client(_ok_handler([]))
    with pytest.raises(MCPToolError) as excinfo:
        st_tool.run(
            client,
            _cfg(),
            {"species_entry_ref": VALID_REF, "pressure": 1.0},
        )
    assert "unknown field" in excinfo.value.detail
    client.close()
...
def test_temperature_filters_forwarded() -> None:
    ...
    p = captured[0]["params"]
    assert p["temperature_min"] == "298.15"
    assert p["temperature_max"] == "1500.0"
    client.close()
...
def test_rejects_non_numeric_temperature() -> None:
    ...
    with pytest.raises(MCPToolError):
        st_tool.run(
            client,
            _cfg(),
            {"species_entry_ref": VALID_REF, "temperature_min": "hot"},
        )
    client.close()
...
def test_rejects_bool_temperature() -> None:
    ...
    with pytest.raises(MCPToolError):
        st_tool.run(
            client,
            _cfg(),
            {"species_entry_ref": VALID_REF, "temperature_min": True},
        )
    client.close()


def test_software_and_model_kind_filters_forwarded() -> None:
    captured: list[dict[str, Any]] = []
    client = _make_client(_ok_handler(captured))
    st_tool.run(
        client,
    ...
    p = captured[0]["params"]
    assert p["software"] == "rmg"
    assert p["model_kind"] == "nasa"
    client.close()
...
def test_server_422_maps_to_invalid_input() -> None:
    def handler(request: httpx.Request) -> httpx.Response:
        return httpx.Response(
            422,
            json={
                "detail": "handle_type_mismatch: expected spe_*",
...
def test_server_404_maps_to_not_found() -> None:
    ...
    client = _make_client(handler)
    with pytest.raises(MCPToolError) as excinfo:
        st_tool.run(client, _cfg(), {"species_entry_ref": "spe_missing"})
...
def test_envelope_returned_unchanged() -> None:
    fixture = _stub_response(
        records=[{"species_entry_ref": "spe_aaa", "model_kind": "nasa"}]
    )
    ...
    client = _make_client(handler)
    result = st_tool.run(client, _cfg(), {"species_entry_ref": VALID_REF})
