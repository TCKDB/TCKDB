...
def create_literature(literature_data: LiteratureCreate, db: Session):
    ...
    if literature_data.type == LiteratureType.book and literature_data.isbn:
        existing_book = (
            db.query(Literature).filter_by(isbn=literature_data.isbn).first()
        )
        if existing_book:
            raise HTTPException(
                status_code=400,
    ...
    if literature_data.type == LiteratureType.article and literature_data.doi:
        existing_article = (
            db.query(Literature).filter_by(doi=literature_data.doi).first()
        )
        if existing_article:
            raise HTTPException(
                status_code=400,
    ...
    literature = Literature(
        ...
        publisher=(
            literature_data.publisher.strip() if literature_data.publisher else None
        ),
        ...
        chapter_title=(
            literature_data.chapter_title.strip()
            if literature_data.chapter_title
            else None
        ),
        publication_place=(
            literature_data.publication_place.strip()
            if literature_data.publication_place
            else None
        ),
...
def update_literature(
    db: Session, literature_id: int, literature_data: LiteratureUpdate
):
    ...
    # Update other fields
    update_data = literature_data.model_dump(exclude_unset=True, exclude={"authors"})
    ...
    if literature_data.authors:
        ...
        existing_authors = {
            (author.first_name.lower(), author.last_name.lower())
            for author in db_literature.authors
        }

        for author_data in literature_data.authors:
            author_tuple = (
                author_data.first_name.strip().lower(),
                author_data.last_name.strip().lower(),
            )
...
def _restore_literature(db: Session, literature_id: int):
    ...
    db_literature = (
        db.query(Literature)
        .with_deleted()
        .filter(Literature.id == literature_id, Literature.deleted_at.isnot(None))
        .first()
