...
def test_lowercase_dict():
    ...
    assert common.lowercase_dict(d_4) == {'d': {'a': 'z', 'v': 'n', 'h': 54, 'f': 'l', 7: 0, 8: {'q': 9}}}
...
def test_is_valid_inchi():
    ...
    assert common.is_valid_inchi('InChI=1S/C19H37NO8/c1-11(27-17-9-16(24-5)14(22)10-26-17)8-15(12(2)21)28-19-18'
                                 '(23)13(20(3)4)6-7-25-19/h11-19,21-23H,6-10H2,1-5H3')[0]
...
def test_is_valid_inchi_key():
    ...
    assert common.is_valid_inchi_key('PXOLZRNDFGGLHD-UHFFFAOYSA-N')[0]  # CCC(S)C(C)C(CF)CCl
    assert common.is_valid_inchi_key('YZCKVEUIGOORGS-UHFFFAOYSA-N')[0]  # [H]
    assert common.is_valid_inchi_key('HSMPSHPWCOOUJH-UHFFFAOYSA-N')[0]  # N=C1[CH]C=CC=C1
...
def test_is_valid_adjlist():
    ...
    assert common.is_valid_adjlist("""multiplicity 2
...
13 H u0 p0 c0 {6,S}""")[0]
    assert common.is_valid_adjlist("""multiplicity 2
1 H u1 p0 c0""")[0]
    assert common.is_valid_adjlist("""1 O u0 p2 c0 {2,S} {3,S}
2 H u0 p0 c0 {1,S}
3 H u0 p0 c0 {1,S}""")[0]
    assert common.is_valid_adjlist("""1  C u0 p0 c0 {2,S} {8,S} {9,S} {10,S}
...
16 H u0 p0 c0 {7,S}
17 H u0 p0 c0 {7,S}""")[0]
    assert common.is_valid_adjlist("""1 F  u0 p3 c0 {2,S}
