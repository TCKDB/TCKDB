...
def test_encorr_schema(supported_elements, valid_aec, valid_bac, primary_level, isodesmic_high_level):
    ...
    assert 'does not seem to correspond to a known chemical element' in str(exc_info.value)
    ...
    assert 'The supported_elements list is missing the symbol "Si"' in str(exc_info.value)
    ...
    assert 'supported_elements list length and the number of entries in aec must be equal' in str(exc_info.value)
    ...
    assert 'Either isodesmic reactions or aec and bac arguments must be specified.' in str(exc_info.value)
    ...
    assert 'When specifying isodesmic reactions, both aec and bac arguments must not be specified' in exc_info.value.errors()[0]['msg']
    ...
    assert 'When specifying isodesmic reactions, both aec and bac arguments must not be specified' in str(exc_info.value)
    ...
    assert 'An isodesmic reaction entry must include all four "reactants", "products", "stoichiometry", and "DHrxn298" keys.' in str(exc_info.value)
    ...
    assert 'An isodesmic reaction entry must include all four "reactants", "products", "stoichiometry", and "DHrxn298" keys.' in str(exc_info.value.errors()[1]['msg'])
    ...
    assert 'The isodesmic_high_level must be provided when isodesmic_reactions are specified' in str(exc_info.value)
