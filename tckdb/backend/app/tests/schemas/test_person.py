...
def test_person_schema():
    ...
    person_1 = PersonBase(
        ...
        uploaded_species=300000,
        uploaded_reactions=15000,
        uploaded_networks=500,
    )
    ...
    with pytest.raises(ValidationError):
        # two @ in email
        PersonBase(
            name="I. B. Writing", email="not.a@valid@email", affiliation="Institution"
        )
    with pytest.raises(ValidationError):
        # space in email
        PersonBase(
    ...
    with pytest.raises(ValidationError):
        # space in email
        PersonBase(
            name="I. B. Writing", email="email @dot.com", affiliation="Institution"
        )
    with pytest.raises(ValidationError):
        # no dot in email
        PersonBase(
            name="I. B. Writing",
            email="not_a_valid@emailcom",
            affiliation="Institution",
        )
    with pytest.raises(ValidationError):
        # no space in name
