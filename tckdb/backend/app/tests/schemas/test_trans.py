...
def test_trans_schema():
    ...
    with pytest.raises(ValidationError):
        # wrong model
        TransBase(model='wrong',
                  parameters={'alpha0': (175, 'cm^-1'), 'T0': (300, 'K'), 'n': 0.52})
    with pytest.raises(ValidationError):
        # wrong parameters type
        TransBase(model='Single Exponential Down',
                  parameters=(175, 'cm^-1'))
    with pytest.raises(ValidationError):
        # wrong parameters type
        TransBase(model='Single Exponential Down',
    ...
    with pytest.raises(ValidationError):
        # wrong parameters type
        TransBase(model='Single Exponential Down',
                  parameters={'alpha0': {'a': [175, 'cm^-1']}, 'T0': (300, 'K'), 'n': 0.52})
    with pytest.raises(ValidationError):
        # wrong parameters type
        TransBase(model='Single Exponential Down',
                  parameters={'alpha0': (175, 'cm^-1'), 'T0': 300, 'n': 0.52})
    with pytest.raises(ValidationError):
        # no alpha0
        TransBase(model='Single Exponential Down',
                  parameters={'T0': (300, 'K'), 'n': 0.52})
    with pytest.raises(ValidationError):
        # no T0
        TransBase(model='Single Exponential Down',
                  parameters={'alpha0': (175, 'cm^-1'), 'n': 0.52})
    with pytest.raises(ValidationError):
        # no n
        TransBase(model='Single Exponential Down',
                  parameters={'alpha0': (175, 'cm^-1'), 'T0': (300, 'K')})
    with pytest.raises(ValidationError):
        # unsupported parameter key
        TransBase(model='Single Exponential Down',
                  parameters={'alpha0': (175, 'cm^-1'), 'T0': (300, 'K'), 'n': 0.52, 'unsupported': (175, 'cm^-1')})
    with pytest.raises(ValidationError):
        # non tuple value
        TransBase(model='Single Exponential Down',
                  parameters={'alpha0': (175, 'cm^-1'), 'T0': (300, 'K'), 'n': [0.52]})
    with pytest.raises(ValidationError):
        # long tuple
        TransBase(model='Single Exponential Down',
                  parameters={'alpha0': (175, 'cm^-1'), 'T0': (300, 'K', 'J'), 'n': 0.52})
    with pytest.raises(ValidationError):
        # non float first value
        TransBase(model='Single Exponential Down',
                  parameters={'alpha0': ('hundred seventy five', 'cm^-1'), 'T0': (300, 'K'), 'n': 0.52})
    with pytest.raises(ValidationError):
        # non string second value
        TransBase(model='Single Exponential Down',
                  parameters={'alpha0': (175, {15: 4}), 'T0': (300, 'K'), 'n': 0.52})
    with pytest.raises(ValidationError):
        # units for n
        TransBase(model='Single Exponential Down',
                  parameters={'alpha0': (175, 'cm^-1'), 'T0': (300, 'K'), 'n': (0.52, 'units')})
