...
class TestLiteratureEndpoints:
    ...
    def setup_literature(self, request, client):
        ...
        response = client.post(f"{API_V1_STR}/literature/", json=article_data)
        ...
        book_data = {
            "type": "book",
            "authors": [
                {"first_name": "M.I.", "last_name": "It"},
                {"first_name": "D.C", "last_name": "Wash"},
            ],
        ...
        response = client.post(f"{API_V1_STR}/literature/", json=book_data)
        assert response.status_code == 201, response.text
        data = response.json()
        ...
        response = client.post(f"{API_V1_STR}/literature/", json=thesis_data)
        assert response.status_code == 201, response.text
        data = response.json()
    ...
    def get_literature_from_db(self, literature_id, db):
        ...
        return (
            db.query(LiteratureModel)
            .filter(LiteratureModel.id == literature_id)
            .first()
        )
    ...
    def test_create_literatures(self):
        ...
        assert self.article_data["authors"] == [
        ...
        assert self.book_data["authors"] == [
            {"id": 3, "first_name": "M.I.", "last_name": "It"},
            {"id": 4, "first_name": "D.C", "last_name": "Wash"},
        ]
        assert self.book_data["title"] == "Principles of Kinetic Modeling"

        assert self.thesis_data["type"] == "thesis"
        assert self.thesis_data["authors"] == [
            {"id": 5, "first_name": "P.H.", "last_name": "David"}
        ]
    ...
    def test_update_literature_partial(self, client):
        ...
        assert updated_data["authors"] == [
            {"id": 1, "first_name": "John", "last_name": "Doe"},
            {"id": 2, "first_name": "Jane", "last_name": "Doe"},
            {"id": 6, "first_name": "John", "last_name": "Smith"},
        ]
        ...
        response_book = client.patch(
            ...
            json={
                ...
                "authors": [
                    {"first_name": "M.I.", "last_name": "It"},
                    {"first_name": "D.C", "last_name": "Wash"},
                    {"first_name": "J.P", "last_name": "Morgan"},
                ],
            },
        )
        assert response_book.status_code == 200, response_book.text
        ...
        assert updated_data["authors"] == [
            {"id": 3, "first_name": "M.I.", "last_name": "It"},
            {"id": 4, "first_name": "D.C", "last_name": "Wash"},
            {"id": 7, "first_name": "J.P", "last_name": "Morgan"},
        ]
        ...
        response_thesis = client.patch(
            ...
            json={
                ...
                "authors": [
                    {"first_name": "J.P", "last_name": "Morgan"},
                    {"first_name": "Jim", "last_name": "David"},
                ],
            },
        )
        assert response_thesis.status_code == 200, response_thesis.text
        ...
        assert updated_data["authors"] == [
            {"id": 5, "first_name": "P.H.", "last_name": "David"},
            {"id": 7, "first_name": "J.P", "last_name": "Morgan"},
            {"id": 8, "first_name": "Jim", "last_name": "David"},
        ]
    ...
    def test_soft_delete_literature(self, client, db_session):
        ...
        # Soft delete article
        response_article = client.delete(
            f"{API_V1_STR}/literature/{self.article_id}/soft"
        )
        assert response_article.status_code == 200, response_article.text
        ...
        # Soft delete thesis
        response_thesis = client.delete(
            f"{API_V1_STR}/literature/{self.thesis_id}/soft"
        )
        assert response_thesis.status_code == 200, response_thesis.text
    ...
    def test_restore_literature(self, client, db_session):
        ...
        # Restore article
        response_article = client.post(
            f"{API_V1_STR}/literature/{self.article_id}/restore"
        )
        assert response_article.status_code == 200, response_article.text
        db_article = self.get_literature_from_db(self.article_id, db_session)
        assert db_article is not None
        ...
        # Restore thesis
        response_thesis = client.post(
            f"{API_V1_STR}/literature/{self.thesis_id}/restore"
        )
        assert response_thesis.status_code == 200, response_thesis.text
        db_thesis = self.get_literature_from_db(self.thesis_id, db_session)
        assert db_thesis is not None
