...
def test_freq_model():
    ...
    assert (
        repr(freq1)
        == "<FreqScale(id=None, factor=1.00386, level_id=None, source='J.A. Montgomery, M.J. Frisch, "
    ...
    assert (
        str(freq2)
        == "<FreqScale(factor=0.98, level_id=None, source='Calculated using the Truhlar method')>"
    )
