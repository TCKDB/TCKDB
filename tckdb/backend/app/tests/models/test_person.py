...
def test_person_model():
    ...
    assert (
        str(person_1)
        == "<Person(name='I. B. Writing', email='email@dot.com', affiliation='Institution', "
        "uploads=30, reviews=15)>"
    )
    assert (
        repr(person_1)
        == "<Person(id=None, name='I. B. Writing', email='email@dot.com', "
    ...
    person_2 = Person(
        ...
        email="email@dot.com",
        affiliation="Institution",
        uploaded_species=3000,
        uploaded_non_physical_species=1,
        uploaded_reactions=500,
        uploaded_networks=35,
        reviewed_species=1500,
        reviewed_non_physical_species=5,
        reviewed_reactions=60,
        reviewed_networks=10,
    )
