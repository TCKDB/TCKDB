...
class FreqScaleBase(BaseModel):
    ...
    """
    factor: Optional[float] = Field(None, gt=0, lt=2, title='The frequency scaling factor')
    source: Optional[str] = Field(None, max_length=1600, title='The source of method used to derive this frequency scaling factor')
...
class FreqScaleCreateBatch(FreqScaleBase, ConnectionBase):
    ...
    level_connection_id: Optional[str] = Field(None, title='The level of theory connection id')
