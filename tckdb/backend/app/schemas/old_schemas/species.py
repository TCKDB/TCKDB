...
class SpeciesBase(BaseModel):
    ...
    reviewed: Optional[bool] = Field(
        None, title="Whether the species has been reviewed"
    )
...
class SpeciesCreate(SpeciesBase):
    ...
    def frequencies_validator(cls, value, values: ValidationInfo):
        ...
        if value is not None:
            ...
            if values["coordinates"] is not None and value is not None:
                linear = is_linear(
                    coordinates=np.array(values["coordinates"]["coords"])
                )
            ...
            if (
                "is_ts" in values
                and values["is_ts"]
                and all(freq > 0 for freq in value)
            ):
                raise ValueError(
    ...
    def check_descriptor_presence(cls, values: ValidationInfo):
        """
        Check that at least one of the species descriptors is provided
        """
        smiles, inchi, graph = (
            values.data.get("smiles"),
            values.data.get("inchi"),
    ...
    def graph_validator(cls, value, values: ValidationInfo):
        ...
        if (
            values.data.get("inchi_key") is not None
            and values.data.get("inchi") is None
        ):
            ...
            if values["inchi"] is not None:
                ...
                value = value or converter.adjlist_from_smiles(values["smiles"])
        values["inchi_key"] = values.data.get(
            "inchi_key"
    ...
    def fragments_validator(cls, value, values: ValidationInfo):
        ...
        if (
            "coordinates" in values
            and values["coordinates"]
            and len(values["coordinates"]["symbols"]) != len(atom_indices)
        ):
            raise ValueError(
    ...
    def freq_id_validator(cls, value, values: ValidationInfo):
        ...
        if (
            "frequencies" in values
            and values["frequencies"] is not None
            and value is None
        ):
    ...
    def cp_t_list_validator(cls, value, values: ValidationInfo):
        ...
        if (
            "Cp_values" in values
            and values["Cp_values"]
            and value is not None
            and len(values["Cp_values"]) != len(value)
        ):
            raise ValueError(
                f"The number of Cp values ({len(values['Cp_values'])}) "
