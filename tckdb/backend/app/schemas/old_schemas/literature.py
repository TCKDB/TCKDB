...
class LiteratureBase(BaseModel):
    ...
    def check_journal(cls, value, values):
        ...
        if 'type' in values and values['type'] == LiteratureType.article and (value is None or not value):
    ...
    def check_publisher(cls, value, values):
        ...
        if 'type' in values and values['type'] == LiteratureType.book and (value is None or not value):
    ...
    def check_advisor(cls, value, values):
        ...
        if 'type' in values and values['type'] == LiteratureType.thesis and (value is None or not value):
    ...
    def validate_url(cls, value, values):
        ...
        if 'type' in values and values['type'] == LiteratureType.thesis or value is not None:
            if '.' not in value:
...
class LiteratureCreateBatch(TempBase, LiteratureCreate):
    ...
    author_temp_ids: List[str] = Field(None, title='A list of temporary IDs for the authors of the literature source')  # New authors
