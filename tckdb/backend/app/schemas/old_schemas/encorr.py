...
class EnCorrBase(BaseModel):
    ...
    supported_elements: Optional[List[str]] = Field(
        None, title="The chemical elements supported by this energy correction object"
    )
    energy_unit: Optional[str] = Field(
        None, max_length=255, title="The energy units the corrections are given in"
    )
    ...
    isodesmic_reactions: Optional[List[IsodesmicReactionEntry]] = Field(
        None, title="Isodesmic reactions"
    )
    ...
    primary_level_temp_id: Optional[int] = Field(
        None, title="The primary level of theory temp id"
    )
    isodesmic_high_level_temp_id: Optional[int] = Field(
        None,
        title="The high level of theory temp id used in the isodesmic reactions correction",
    )
    ...
    @field_validator("supported_elements", pre=True, mode="before")
    def elements_exist(cls, value):
        ...
        for symbol in value:
            ...
            if not is_valid:
                raise ValueError(
                    f"The symbol {symbol} in {value} does not seem to correspond to a known "
    ...
    def validate_energy_unit(cls, value):
        ...
        if not is_valid:
            raise ValueError(
                f'The energy unit "{value}" does not seem to be a valid energy unit. Reason:\n{err}'
            )
        return value
            ...
            for isodesmic_reaction in value:
                for key, val in isodesmic_reaction.items():
                    if key in ["reactants", "products"]:
                        ...
                        raise ValueError(
                            f'Allowed keys in an isodesmic reaction are "reactants", "products", '
                            f'"stoichiometry", and "DHrxn298". Got {key}.'
                        )
                if len(list(isodesmic_reaction.keys())) != 4:
                    raise ValueError(
                        f'An isodesmic reaction entry has to include all three "reactants", "products", '
...
class EnCorrCreate(EnCorrBase):
    ...
    isodesmic_reactions: Optional[List[Dict[str, Union[list, float]]]] = Field(
        None, title="Isodesmic reactions"
    )
    reviewer_flags: Optional[Dict[str, str]] = Field(None, title="Reviewer flags")
    ...
    isodesmic_high_level: Optional[LevelCreate] = Field(
        None, title="High level of theory information for isodesmic reactions"
    )
    model_config = ConfigDict(from_attributes=True, extra="forbid")
...
class EnCorrUpdate(EnCorrBase):
    ...
    isodesmic_reactions: Optional[
        List[Dict[str, Union[List[str], List[int], float]]]
    ] = None
    reviewer_flags: Optional[Dict[str, str]] = None

    level_id: Optional[LevelUpdate] = Field(
        None, title="The level of theory id from the Level table"
    )
    isodesmic_high_level_id: Optional[LevelUpdate] = Field(
        None,
        title="The high level of theory id from the Level table used in the isodesmic reactions correction",
    )
    model_config = ConfigDict(from_attributes=True, extra="forbid")
...
class EnCorrOut(EnCorrInDBBase):
    ...
    primary_level: Level
    isodesmic_high_level: Optional[Level] = None
...
class EnCorrInDB(EnCorrInDBBase):
    ...
    isodesmic_high_level: Level = Field(
        None, title="High level of theory information for isodesmic reactions"
    )
    species: List[SpeciesRead] = Field(
        [], title="Species associated with this energy correction object"
    )
