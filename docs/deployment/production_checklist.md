...
| `DB_CONNECT_TIMEOUT_SECONDS` | `10` (the default) | Caps how long libpq spends opening one connection. Distinct from `DB_STATEMENT_TIMEOUT_MS`, which is server-side and so bounds nothing until a server is on the far end of a socket. It matters for the failure mode where the database host is *black-holed* rather than down — a firewall rule, a departed container, a stale DNS answer — which returns after the kernel's SYN retries (~130 s here) rather than at once. Raise it only if the database is genuinely far away; `0` disables it and restores the OS default. |
...
- [ ] `GET /` against the API itself returns `404` — there is no API-side landing page: `curl -s -o /dev/null -w '%{http_code}\n' http://127.0.0.1:8010/` from the host the API runs on (or `docker compose exec <api service> curl ...` if your recipe containerizes it).
- [ ] If a frontend is deployed in front of this API (Mode 3 / hosted-community — see [deployment_modes.md](deployment_modes.md)), `GET /` at the **public** base URL returns `200 text/html`, served by the frontend SPA, not the API: `curl -s -o /dev/null -w '%{http_code}\n' https://your-host/`. A human following the base URL from a paper must not be met with a JSON 404. (Mode 2 self-hosted recipes without a frontend container are exempt from this row; see [self_hosted_single_node.md](self_hosted_single_node.md).)
