...
- [ ] `GET /` against the API container itself returns `404` — there is no API-side landing page (the module was deleted; it rendered only on the container's loopback address and nothing forwarded to it). A human following the base URL from a paper reaches the frontend SPA's `/` through nginx, which is a separate container and out of scope for this checklist.
