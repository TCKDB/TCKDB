...
| Working precedent helper | `persist_artifact` in [backend/app/services/artifact_persistence.py](../../backend/app/services/artifact_persistence.py) | ✅ decode → validate → store → row creation. (Was a private `_persist_artifact` inside `app/workflows/computed_reaction.py`; it has since been extracted into the shared service module alongside `persist_artifact_batch`.) |
