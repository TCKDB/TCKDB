...
| `reference_pressure_bar` | DOUBLE PRECISION | yes | — | — | — | ``reference_pressure_bar`` is the standard-state pressure the H/S and NASA/tabulated values are referenced to (IUPAC 1 bar; legacy data using the older 1 atm convention should record 1.01325). ``NULL`` means the reference pressure was not specified. |
