...
**Status: accepted 2026-08-08.** Implemented in `tckdb_schemas.stationary_point`, which supersedes the `n_imag == 1` blocking rule for transition states. Does not change the rules for minima or van der Waals complexes. Three things this document asserted before implementation turned out to be wrong or unbuildable; each is corrected in place below and listed under [What implementation changed](#what-implementation-changed). **Extended 2026-09-04** by [§"Amendment 2026-09-04"](#amendment-2026-09-04-an-assumed-default-when-the-method-is-not-recorded) — an addition, not a correction: the original decision stands unchanged, and the amendment only narrows how often its conservative fallback is reached.
...
## Amendment 2026-09-04: an assumed default when the method is not recorded

**§"What implementation changed" said this plainly and it stayed true for three weeks:** "Gaussian does not name its default, so an unqualified `Freq` leaves the key absent and takes the conservative row." What had not been measured is *how much* of the corpus that sentence actually describes. Measured 2026-09-04, against the live deployed archive: **0 of 132 stored `calc_freq_result` rows carry `freq.hessian_method`.** Not "mostly Gaussian, occasionally absent" — every single one. The conservative row was written to describe the rare case of an honestly ambiguous record; in practice it was the whole corpus, because Gaussian never states its second-derivative method in the output for the common case (analytic), and the parser correctly refuses to infer one from silence. τ = 50 cm⁻¹, the finite-difference-from-gradients row, on every Gaussian frequency job TCKDB has ever stored, including the ordinary DFT single-reference case where Gaussian has computed an analytic Hessian by default since at least G03.

That is not a bug in the parser — "record only explicit statements" was the right call, and stays the right call for what counts as *recorded* provenance. It is a gap this amendment closes a different way: not by inferring more from the output, but by letting TCKDB **assume** the producing program's own documented default, on the record, distinguishably from a recorded statement.

**The decision (the archive owner's, 2026-09-04):** when a frequency job's Hessian method is not recorded, TCKDB may assume the producing program's documented default for the level-of-theory's method family, record that it was *assumed* rather than recorded, and give the assumed method the same τ as its recorded counterpart.

**Same τ, on purpose.** This was the one point that needed an explicit decision rather than falling out of the existing design: an assumption is weaker evidence than a recorded statement, so the naive instinct is to interpolate — give it a τ between the assumed row and `protocol_not_recorded`. That would have reopened exactly the table this ADR's 2026-08-15 amendment retired for being "plausible, and that is all it was." The owner's ruling keeps faith with that earlier finding: τ's job is to separate a quiet warning from a flagged one, and the amendment does not add a second numeric axis for "how sure are we this is analytic." The one already-published number for "analytic Hessian, default grid" (30 cm⁻¹) is either the right τ for this case or the existing table is wrong about what analytic-default means, and nothing in this amendment's evidence says the latter.

**The table.** Gaussian: analytic for HF, any DFT functional, MP2, CIS and CASSCF (none of these have ever required `Freq=Numer` to get a frequency job at these levels); finite-difference-from-gradients for MP3, MP4 (any truncation), QCISD and CCSD, none of which have an analytic Hessian in Gaussian but all of which have an analytic gradient to differentiate; finite-difference-from-energies for CCSD(T) and QCISD(T), which have neither. ORCA: analytic for HF and any DFT functional; finite-difference for the correlated-wavefunction methods that have no analytic Hessian there (MP2 and its variants, coupled cluster, every `DLPNO-*` method) — ORCA's own default `NumFreq` differentiates the analytic gradient these methods do have, so the amendment does not split a gradient/energy sub-case for it the way Gaussian's table does. Implemented as `app/services/hessian_method_inference.py`, a pure, table-driven function taking a software name and a level-of-theory method and returning an assumed resolution or `None`; a program or method pair not in the table returns `None` and the record stays at the conservative `protocol_not_recorded` row, unchanged. This is the same conservatism §"The decision" has always applied to an unrecorded method — the amendment narrows *when* the conservative row is reached, it does not remove it.

**The basis carries the assumption, distinguishably.** `TauBasis` (`tckdb_schemas.stationary_point`) gained three members — `assumed_analytic_default`, `assumed_finite_difference_gradient`, `assumed_finite_difference_energy` — rather than reusing the three recorded ones. This is the load-bearing invariant the owner stated alongside the decision: **an assumption must never be confusable with a recorded statement.** A reader who filters on basis, or who reads `freq.hessian_method` beside the stored basis (as `TauContext.protocol_parameters` already does — see §"What implementation changed" above), sees the assumption for what it is: the basis says `assumed_analytic_default`, and the recorded parameter for `freq.hessian_method` is genuinely absent, because nothing about assuming a default manufactures a parameter row that was never observed. `ck_calc_freq_result_imaginary_mode_tau_basis_known` (added by `e2a7c9d4b615`, §"What was actually built" analogue) is widened to the eight-value vocabulary by a new revision (`ae3d607cc9f8`), not folded into the revision that closed it — the deployed-table migration rule applies here exactly as it does to every other schema change on `calc_freq_result`.

**A recorded method always supersedes an assumption.** The assumption is consulted only when `tau_resolution()` — the same function this ADR's §"What implementation changed" describes, unmodified — has already resolved to `protocol_not_recorded` from this calculation's own parameters. A depositor (or a later re-upload) that states `freq.hessian_method` explicitly is read by that same function exactly as before and wins outright; the inference table is never given the chance to override a recorded statement, because it never runs on that path at all. This is the same asymmetry §"τ never decides between blocking and warning" already relies on: missing provenance changes how a record is classified, never what a recorded statement says.

**Existing records, not just new ones.** `backend/scripts/ops/backfill_assumed_tau.py` applies the identical resolution — recorded parameters first, then the assumption table — to every `calc_freq_result` row with `imaginary_mode_tau_cm1 IS NULL`, which after this amendment means every pre-amendment row still sitting at "never judged." It writes only `imaginary_mode_tau_cm1` and `imaginary_mode_tau_basis`; `imaginary_mode_structural_flag` is left exactly as this ADR's §"Consequences" already argues it should be — judged only where a reaction-coordinate mode is designated, which nothing in this script's scope has, because designation postdates every row it reaches. Dry-run by default, idempotent (the scope predicate `IS NULL` is false on every row it writes), and refuses `--apply` against anything that does not look like a test database unless explicitly told the target is deployed. Run against the hosted archive, this is the mechanism that turns "0 of 132 have a basis better than `protocol_not_recorded`" into "132 of 132 have the best basis TCKDB can honestly assign them" — not by learning anything new about any of the 132 records, but by asking the one question this ADR always had the machinery to answer and had simply never asked: what does the producing program's own documentation say it does by default.
