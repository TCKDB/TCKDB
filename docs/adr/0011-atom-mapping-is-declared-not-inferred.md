...
What a depositor can actually do about a reaction already deposited without a map — and what re-depositing, the supersession route, and the idempotency key each do — is written up separately in [`backend/docs/specs/late_atom_maps.md`](../../backend/docs/specs/late_atom_maps.md). That document decides nothing; it records the composition of rules this decision sets in motion, and the options for the already-deposited corpus.
