...
Every class here (``AppliedEnergyCorrectionUploadPayload``,
...
flows. There is no standalone ``/uploads/energy-corrections`` route —
that is a backend concern.

The frequency-scale-factor ref lives at
``tckdb_schemas.fragments.refs.FreqScaleFactorRef`` and is shared with
the statmech upload path; it is the single ref shape for all FSF use
cases.
"""
