...
class TestThermoScalar:
    ...
    def test_temperature_bounds_must_be_positive(self):
        with pytest.raises(TCKDBBuilderValidationError):
            Thermo.scalar(enthalpy_reference_kind="formation_298k", h298_kj_mol=0.0, tmin_k=0)
        with pytest.raises(TCKDBBuilderValidationError):
            Thermo.scalar(enthalpy_reference_kind="formation_298k", h298_kj_mol=0.0, tmax_k=-1)

    def test_tmin_must_be_le_tmax(self):
        with pytest.raises(TCKDBBuilderValidationError):
            Thermo.scalar(enthalpy_reference_kind="formation_298k", h298_kj_mol=0.0, tmin_k=2000, tmax_k=300)

    def test_to_payload_emits_only_supplied_keys(self):
        t = Thermo.scalar(enthalpy_reference_kind="formation_298k", h298_kj_mol=-74.6, tmax_k=2000)
        payload = t.to_payload()
        assert payload == {"enthalpy_reference_kind": "formation_298k", "h298_kj_mol": -74.6, "tmax_k": 2000.0}
...
class TestThermoNasa:
    ...
    def test_nasa_happy_path(self, coeffs):
        t = Thermo.nasa(
            enthalpy_reference_kind="formation_298k", coeffs_low=coeffs, coeffs_high=coeffs,
    ...
    def test_coeffs_must_be_length_7(self, coeffs):
        with pytest.raises(TCKDBBuilderValidationError):
            Thermo.nasa(
                enthalpy_reference_kind="formation_298k", coeffs_low=[1.0, 2.0],
                coeffs_high=coeffs,
                t_low=200, t_mid=1000, t_high=5000,
            )
        with pytest.raises(TCKDBBuilderValidationError):
            Thermo.nasa(
                enthalpy_reference_kind="formation_298k", coeffs_low=coeffs,
                coeffs_high=[1.0],
                t_low=200, t_mid=1000, t_high=5000,
            )
    ...
    def test_coeffs_must_be_numeric(self, coeffs):
        with pytest.raises(TCKDBBuilderValidationError):
            Thermo.nasa(
                enthalpy_reference_kind="formation_298k", coeffs_low=["zero"] + [1.0] * 6,
                coeffs_high=coeffs,
                t_low=200, t_mid=1000, t_high=5000,
            )
...
class TestThermoPoints:
    def test_points_happy_path(self):
        t = Thermo.points(
            ...
            enthalpy_reference_kind="formation_298k", tmin_k=200, tmax_k=2000,
        )
...
class TestThermoSourceCalculations:
    def test_dict_form(self):
        opt = _opt()
        t = Thermo.scalar(
            enthalpy_reference_kind="formation_298k", h298_kj_mol=0.0,
    ...
    def test_to_payload_omits_source_calculations_by_default(self):
        ...
        opt = _opt()
        t = Thermo.scalar(enthalpy_reference_kind="formation_298k", h298_kj_mol=0.0, source_calculations={"opt": opt})
